<template>
  <div class="pro-table-wrapper">
    <!-- 搜索条件区域 -->
    <SearchForm ref="searchFormRef" :toggleFoldCallBack="toggleFoldCallBack" :updateTableList="updateTableList"
      :conditionList="conditionList" :hiddenSearch="hiddenSearch" :isOpenPage="isOpenPage" :treeList="treeList"
    ...
    <div class="pro-table">
            ...
            <div class="column-wrapper">
              <div class="top-btn-group">
                <span class="top-title"><el-checkbox v-model="selectAll" :indeterminate="isIndeterminate"
                    @change="handleCheckAllChange"></el-checkbox><span class="title-text">显示列</span></span>
      ...
      <el-table ref="elTableRef" :data="tableData" class="pro-table-eleTableBox" :header-cell-style="() => ({
        ...
        v-loading="loading" element-loading-text="正在加载" @selection-change="selectionChange" :border="false">
        <!-- 默认插槽 -->
        ...
        <template v-for="item in columns" :key="item.id">
          ...
          <!-- tag标签 -->
          <el-table-column v-else-if="item.type === 'tag'" :prop="item.prop" :label="item.label" align="center"
            :sortable="item.sortable ?? false" :width="item.width ?? 'auto'" :fixed="item.fixed ?? false">
            <template #default="scope">
              <div class="tag-group">
                <el-tag :type="item.tagColor[scope.row[item.keyProp]]" round v-if="scope.row[item.prop]">{{
                  scope.row[item.prop]
                }}</el-tag>
              </div>
            </template>
          ...
          <!-- tags标签 -->
          <el-table-column v-else-if="item.type === 'tags'" :prop="item.prop" :label="item.label" align="center"
            :sortable="item.sortable ?? false" :width="item.width ?? 'auto'" :fixed="item.fixed ?? false">
            <template #default="scope">
              <div class="tags-group" v-if="scope.row[item.prop]">
                <el-tag type="primary" round v-for="tag in scope.row[item.prop].split(',')" style="margin-right: 4px">
                  {{ tag }}
                </el-tag>
              </div>
            </template>
          ...
          <!-- expand列 -->
          <el-table-column v-else-if="item.type === 'expand'" type="expand" :prop="item.prop" :label="item.label"
            :width="item.width ?? 'auto'" align="center">
            <template #default="scope">
              <div class="content">
                请求参数：{{ scope.row[item.prop] }}
              </div>
            </template>
          </el-table-column>
          <!-- 操作按钮列 -->
          ...
          <el-table-column v-else :prop="item.prop" :label="item.label" :align="item.align ?? 'center'"
            :sortable="item.sortable ?? false" :min-width="item.width ?? 'auto'" :fixed="item.fixed ?? false">
            <template #default="scope">
              {{

<template>
  <div class="chat_layout">
    <div class="chat" style="white-space: pre-line; height: calc(100vh - 110px); overflow-y: auto;">
...
const [agent] = useXAgent({
  request: async ({ message }, { onSuccess, onUpdate }) => {
    const params: {
      role: string;
      content: Attachment[] | { type: string; text: string | undefined | any }[];
    } = {
      role: 'user',
      content: []
    }
...
const handleFileChange = (info: any) => {
  ...
  if (info.file.size) {
    const currentFile = Object.assign(info.file, {}, {
      name: info.file.name,
...
const items = computed<any>(() => {
  return messages.value.map((item) => {
    ...
    const renderFooterContent = {
      local: h(Space, { size: 'small' }, [h(Button, {
        color: 'primary',
        variant: 'outlined',
        size: 'small',
        icon: h(SyncOutlined),
      ...
      loading: null,
      ai: h(Space, { size: 'small' }, [h(Button, {
        color: 'primary',
        variant: 'outlined',
        size: 'small',
        icon: h(CopyOutlined),
...
const fItems = computed(() => {
  return items.value.length > 0 ? items.value : [{ content: placeholderNode, variant: 'borderless' }]
})
...
const roles: BubbleListProps['roles'] = {
  ai: {
    placement: 'start',
    header: "狗尾巴花的尖",
    avatar: { icon: h('img', { src: "https://www.flowertip.site/pro/favicon.svg", alt: 'logo' }), style: { background: '#fff' } },
    ...
    loadingRender: () => h(Space, null, [h(Spin, { size: 'small' }), '我正在整理思路，请稍等...']),
  },
  local: {
    placement: 'end',
